...
from models.patchcore.dynamic_module import DynamicBufferModule
from models.patchcore.extractor import FeatureExtractor
from models.patchcore.k_center_greedy import KCenterGreedy
...
class PatchcoreModel(DynamicBufferModule, nn.Module):
    ...
    def forward(self, input_tensor: Tensor) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        ...
        input_tensor = input_tensor.type(torch.float32)/255
        ...
        if not self.training:
            ...
            #anomaly_scores = anomaly_score[:,None,None,None].repeat(1,1,512,512)
            #output = torch.cat((anomaly_map,anomaly_scores),0)
    ...
    def subsample_embedding(self, embedding: Tensor, sampling_ratio: float, k: int) -> None:
        ...
        # Coreset Subsampling
        sampler = KCenterGreedy(embedding=embedding, sampling_ratio=sampling_ratio, callback = self.callback)
