...
def statistics(y_test,score):
    ...
    for i in range (10000):
        ...
        TN = 0
        for i in range(len(y_test)):
            if ((score[i] >= threshhold) and (y_test[i] == 0)):
                FP +=1
            elif ((score[i] >= threshhold) and (y_test[i] != 0)):
                TP +=1
            elif ((score[i] < threshhold) and (y_test[i] == 0)):
                TN +=1
            else:
    ...
    return recalls,precisions,TPRS,FPRS
...
class SkipDFR(QObject):
    ...
    def build_dataset(self, is_train):
        from models.skipDFR.MVTec import NormalDataset, TestDataset
...
#             print("Done.")
    ...
    def score_img_scale(self, input):
        ...
        input = self.extractor(input)
        dec = self.autoencoder(input)
        score = self.autoencoder.loss_function(dec, input)
        return score
