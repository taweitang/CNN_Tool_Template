...
from models.multiDFR.feature import Extractor
...
from models.multiDFR.feat_cae import FeatCAE
...
def cvt2heatmap(gray,threshold = 0.6):
...
class MultiScaleDFR(QObject):
    ...
    def __init__(self, cfg):
        super(MultiScaleDFR, self).__init__()
        self.cfg = cfg
...
#             print("Done.")
    ...
    def score(self, input):
        ...
        # sample energy
        scores = self.autoencoder.compute_energy(dec[0], dec[1])
    ...
    def detect(self):
        ...
        while True:
            if len(self.detect_imgs)==0:
                ...
                image = cv2.resize(cv2.cvtColor(image, cv2.COLOR_BGR2RGB),self.cfg.featmap_size)
                transform = transforms.Compose([transforms.ToTensor()])
                img = transform(image).float().to(self.device)
                img = img[None,:]
                #img = torch.from_numpy(image).float().to(self.device)
                ...
                #print('image save:' + heatmap_path)
                score = np.sum(anomaly_map)
