...
class FeatCAE(nn.Module):
    ...
    def __init__(self, in_channels=1000, latent_dim=50, is_bn=True):
        ...
        self.encoder3 = nn.Sequential(*layers)
        ...
        # layers += [nn.ReLU()]
        self.decoder3 = nn.Sequential(*layers)

    def forward(self, x):
        in2 = self.encoder1(x)
        in3 = self.encoder2(in2)
        in4 = self.encoder3(in3)
        out1 = self.decoder1(in4)
        out2 = self.decoder2(out1)
        out3 = self.decoder3(out2)
        return [[x,in2,in3],[out1,out2,out3]]
    ...
    def loss_function(self, x, x_hat):
        ...
        for i in range(len(x)):
            loss+=torch.mean((x[i] - x_hat[2-i]) ** 2)
        loss = loss/len(x)
    ...
    def compute_energy(self, x, x_hat):
        ...
        size = x[0][0].shape[-1]
        for i in range(len(x)):
            a_map.append(F.interpolate(x[i], size=size, mode='bilinear', align_corners=True))
            b_map.append(F.interpolate(x_hat[2-i], size=size, mode='bilinear', align_corners=True))
        a_map = torch.cat(a_map,1)
        b_map = torch.cat(b_map,1)
        loss = torch.mean((a_map - b_map) ** 2, dim=1)
